...
info_list_detail = (sw1_show_cdp_neighbors_detail,r1_show_cdp_neighbors_detail,r2_show_cdp_neighbors_detail,r3_show_cdp_neighbors_detail,r4_show_cdp_neighbors_detail,r5_show_cdp_neighbors_detail)

#sw1_show_cdp_neighbors_detail = filter(None,sw1_show_cdp_neighbors_detail)
#sw1_neighbors = sw1_show_cdp_neighbors_detail.split('--------------------------')


def router_info(device_info):
  device_info = filter(None,device_info)
  neighbor_info = device_info.split('--------------------------')

  for a in neighbor_info:
    lines = a.split('\n')
    hostname, ip, model, vendor, device_type = '','','','',''
    
    for line in lines:
      if 'Device ID:' in line:
        hostname = line.split('Device ID:')[1]
        hostname = hostname.strip()
        if hostname not in nw_devices.keys():
          #print 'adding device %s to dictionary as key' %hostname
          nw_devices[hostname] = {}
        #else:
          #print 'device id %s already in dictionary' %hostname
      
      if 'IP address:' in line:
        ip = line.split('IP address:')[1]
        nw_devices[hostname]['ip_addr'] = ip.strip()
        #print hostname
        #print ip
      ...
      if 'Platform' in line:
        ...
        vendor = line.split()[1]
        vendor = vendor.strip()
        nw_devices[hostname]['model'] = model
        nw_devices[hostname]['vendor'] = vendor  
        ...
        for cap in line.split():
          ...
          if 'Router' not in line and 'Switch' not in line:
            capabilities.append('Unknown')
          remove_duplicates(capabilities)
          nw_devices[hostname]['capabilities'] = capabilities


#def connected_devices(device_info):
  
for i in info_list_detail:
   router_info(i)

def parse_cdp(string):
  cdp_str = string.split('Port ID')[1]
  hostname = string.split('show')[0]
  hostname = hostname[:-1]
  hostname = hostname.strip()
  cdp_dict = {}
  cdp_dict['connected devices'] = cdp_str
  cdp_dict['local device'] = hostname
  return cdp_dict



def get_connected(string):
  connected = []
  a = parse_cdp(string)['connected devices']
  b = parse_cdp(string)['local device']
  a = a.split('\n')
  a = filter(None,a) 
  for i in a:
    #print i
  ...
  if b in nw_devices.keys():
    nw_devices[b]['connected devices'] = connected
    
cdp_list = (r5_show_cdp_neighbors,r4_show_cdp_neighbors,r3_show_cdp_neighbors,r2_show_cdp_neighbors,r1_show_cdp_neighbors,sw1_show_cdp_neighbors)
